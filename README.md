...
* 特殊更新版本，包含实验性的更改。
* 修复此模组无法在普通环境下运行的问题
* 删除了食用虫子几率中毒的特性。已证实：此特性不存在于1.6.4 MITE。
* 调整了南瓜派的合成配方，现在额外需要一个面粉
* 调整了蘑菇煲的合成配方，现在需要水碗而不是空碗
* 更改了有关玻璃的合成配方，现在9个玻璃碎片只能合成一块玻璃板；一块玻璃现在可以合成8个玻璃板（而不是1.6.4 MITE的6个）
* 更改了蛋糕的合成配方，现在需面粉、糖、鸡蛋、牛奶碗无序合成。注：作者仔细考虑过是否允许奶桶，但因为这样合成过于冤种（浪费了3/4桶），所以并未添加。你只能使用牛奶碗。
* 添加了以下配方：一个奶桶（水桶）与四个碗可合成四碗奶（水），并返还一个空桶。
* 添加了鞍的合成配方。需要注意：此版本中鞍的配方略微更改，现在中间一行的左右两个物品是皮革绳而非皮革。
* 添加了附魔金苹果的合成配方，现在其可以使用一个附魔之瓶、一个金苹果合成。
* 更改了附魔之瓶被砸碎后给予的经验，现在固定为200。
* 更改了曲奇的合成配方，现在需面团、巧克力无序合成。
...
* 更改了下界合金锭的合成配方，现在需4个残骸碎片和4个金粒合成
* 再次更改了锻造台的合成配方，现在需一把铁剪刀、一把铁锤、任意木板、一个铁锭合成。请不要吐槽其昂贵的造价——或许它只适合在后期被制造出来；而事实的确是这样，因为无法在前期不作弊地获取下界合金制品。
